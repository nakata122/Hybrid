...
List **hash_grid;
List *hitpoints = NULL;
std::vector<HitPoint*> nohit;
std::vector<HitPoint*> veryhit;
AABB hitpointsBB;
    ...
    if (intersection.object->type == 1) {
        ...
        #pragma omp critical
                ...
                while (hitpoints != NULL) {
                    ...
                    if (!hitpoint->light) {
                        ...
                        if ((hitpoint->normal.dot(N) > 1e-3) && (diff.dot(diff) <= hitpoint->radius)) {
                            ...
                            if(depth == 0 && hitpoint->photonCount <= 3) veryhit.push_back(hitpoint);
            ...
            if (RND2 < intersection.object->cl.max()) {
                if(nohit.size() != 0){
                    HitPoint *hip = nohit[std::max(0, (int)(RND2*nohit.size())-1)];
                    if(acos((hip->position - ray.o).norm().dot(N)) < 90 && depth >= 1){
                        ray.d = (hip->position - ray.o).norm();
                    } else{
                        Vec rotX, rotY;
                        ons(N, rotX, rotY);
    ...
    #pragma omp parallel for schedule(dynamic)
        for (int j = 0; j < 1000; j++) {
            if (running) {
                ...
                if(RND2 >= 0.5 && veryhit.size() > 10){
                    HitPoint *hip = veryhit[std::max(0, (int)(RND2*veryhit.size())-1)];
                    ray.d = (hip->position - ray.o).norm();
                }else{
                    Vec rotX, rotY;
                    ons(ray.d, rotX, rotY);
        ...
        while (lst != NULL) {
            ...
            if (hp->light) {
                ...
                if(hp->photonCount <= 2 || (hp->flux.x + hp->flux.y + hp->flux.z)/3 <= 0.3) nohit.push_back(hp);
